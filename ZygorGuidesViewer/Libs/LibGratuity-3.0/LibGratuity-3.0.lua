...
local vmajor, vminor = "LibGratuity-3.0", 90000 + tonumber(string.match("$Revision: 39 $", "%d+"))
...
function lib:CreateSetMethods()
	for _,m in pairs(methods) do
		...
		self[meth] = function(self, ...)
			self:Erase()
			if not func(unpack(arg)) then return end
			return handlePcall(pcall(self.vars.tooltip[meth], self.vars.tooltip, unpack(arg)))
		end
	end
end
...
local function createCompat()
	...
	local Gratuity20 = setmetatable({}, {__index=function(self, key)
		-- AceLibrary probes optional members while registering the legacy
		-- Gratuity-2.0 facade.  Returning self[key] for an absent member re-enters
		-- this __index function forever on Lua 5.0 and ends in a stack overflow.
		-- Cache known members with rawset and return missing members with rawget.
		local value = lib[key]
		if type(value) == "function" then
			local method = value
			self[key] = function(self, ...)
				return method(lib, unpack(arg))
			end
		else
