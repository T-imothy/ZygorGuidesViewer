...
Note:
	...
	http://www.fsf.org/licensing/licenses/gpl-faq.htmlZGV_len(InterpreterIncompat)
]]
...
function Astrolabe:PlaceIconOnMinimap( icon, continent, zone, xPos, yPos )
	...
	minimapRotationEnabled = false -- rotateMinimap CVar does not exist in WoW 1.12.1
	if ( minimapRotationEnabled ) then
...
do
	...
	function Astrolabe:UpdateMinimapIconPositions()
		if ( fullUpdateInProgress ) then
			...
			UpdateMinimapIconPositions(self)
		end
	end
end
...
do
	...
	function Astrolabe:CalculateMinimapIconPositions( reset )
		if reset then resetFullUpdate = true end
		CalculateMinimapIconPositions(self)
		if fullUpdateInProgress then return 1 end
		if resetIncrementalUpdate then return 0 end
		return -1
	end
end
...
function Astrolabe:OnShow( frame )
	frame = frame or self.processingFrame
...
local function activate( newInstance, oldInstance )
	...
	-- WoW 1.12 executes frame scripts with the legacy globals
	-- this, event and arg1 rather than passing modern callback arguments.
	frame:SetScript("OnEvent",
		function()
			Astrolabe:OnEvent(this, event, arg1);
		end
	);
