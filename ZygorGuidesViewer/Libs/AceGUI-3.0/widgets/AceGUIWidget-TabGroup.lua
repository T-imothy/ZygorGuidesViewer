...
local WotLK = false --select(4, GetBuildInfo()) >= 30000
...
do
	...
	local function BuildTabs(self)
		...
		for i = ZGV_len(widths), 1, -1 do 
			widths[i] = nil 
		end
		for i = ZGV_len(rowwidths), 1, -1 do
			rowwidths[i] = nil 
		end
		for i = ZGV_len(rowends), 1, -1 do 
			rowends[i] = nil 
		end
		...
		local numtabs = ZGV_len(tablist)
		local numrows = 1
		local usedwidth = 0

		for i = 1, ZGV_len(tablist) do
		...
		rowwidths[numrows] = usedwidth + 10 --first tab in each row takes up an extra 10px
		rowends[numrows] = ZGV_len(tablist)
