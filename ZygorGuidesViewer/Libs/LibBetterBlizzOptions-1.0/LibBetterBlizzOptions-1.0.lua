...
-- Vanilla 1.12 has no Wrath-era InterfaceOptionsFrame UI. This helper is optional.
if not InterfaceOptionsFrame or not InterfaceOptionsFrameCategories or not InterfaceOptionsFrameAddOns then return end
...
local function updateScrollHeight(categoryFrame)
    ...
    local numButtons = ZGV_len(buttons)
    ...
    if numButtons < maxButtons then
        for i = numButtons + 1, maxButtons do
            ...
            if freeButtons[categoryFrame][i] then
                ...
                button:SetPoint("TOPLEFT", buttons[ZGV_len(buttons)], "BOTTOMLEFT")
            end
