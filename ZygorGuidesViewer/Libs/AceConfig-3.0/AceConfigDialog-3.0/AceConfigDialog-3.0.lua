...
local function safecall(func, ...)
    local callargs = arg
    local function call() return func(unpack(callargs)) end
...
local function pickfirstset(...)
  for i=1,select("#",unpack(arg)) do
    if select(i,unpack(arg))~=nil then
      return select(i,unpack(arg))
    end
  end
end
		...
		for plugin, t in pairs(group.plugins) do
			for k, v in pairs(t) do
				if not opts[k] then
					...
					path[ZGV_len(path)+1] = k
					tempOrders[k] = GetOptionsMemberValue("order", v, options, path, appName)
					tempNames[k] = GetOptionsMemberValue("name", v, options, path, appName)
					path[ZGV_len(path)] = nil
				end
			end
		end
	end
...
function lib:SelectGroup(appName, ...)
	...
	for n = 1, select('#',unpack(arg)) do
		local key = select(n, unpack(arg))
...
local function confirmPopup(appName, rootframe, basepath, info, message, func, ...)
	...
	for i = 1, select('#', unpack(arg)) do
		t[i] = select(i, unpack(arg)) or false
	end
	t.timeout = 0
	t.whileDead = 1
	t.hideOnEscape = 1
...
local function BuildPath(path, ...)
	for i = 1, select('#',unpack(arg))  do
		tinsert(path, (select(i,unpack(arg))))
	end
end
...
function lib:FeedGroup(appName,options,container,rootframe,path, isRoot)
	...
	if hasChildGroups and not inline then
		...
		elseif (parenttype ~= "tree") or isRoot then
			...
			for i = 1, ZGV_len(treedefinition) do
...
function lib:Open(appName, container, ...)
	...
	for n = 1, select('#',unpack(arg)) do
		tinsert(path, (select(n, unpack(arg))))
	end
...
function lib:AddToBlizOptions(appName, name, parent, ...)
	...
	local key = appName
	for n = 1, select('#', unpack(arg)) do
		key = key..'\001'..select(n, unpack(arg))
	end
