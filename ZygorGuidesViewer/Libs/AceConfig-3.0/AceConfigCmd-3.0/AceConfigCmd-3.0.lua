...
local function showhelp(info, inputpos, tab, noHead)
	...
	table.sort(sortTbl, function(one, two) 
		...
		if type(o1) == "function" or type(o1) == "string" then
			info.order = o1
			info[ZGV_len(info)+1] = one
			o1 = callmethod(info, inputpos, refTbl[one], "order")
			info[ZGV_len(info)] = nil
			info.order = nil
		end
		if type(o2) == "function" or type(o1) == "string" then
			info.order = o2
			info[ZGV_len(info)+1] = two
...
local function handle(info, inputpos, tab, depth, retfalse)
	...
	elseif tab.type=="select" then
		...
		if str == "" then
			...
			print(L["Options for |cffffff78"..info[ZGV_len(info)].."|r:"])
	...
	elseif tab.type=="multiselect" then
		...
		if str == "" then
			local fmt = "|cffffff78- [%s]|r %s"
			local fmt_sel = "|cffffff78- [%s]|r %s |cffff0000*|r"
			print(L["Options for |cffffff78"..info[ZGV_len(info)].."|r (multiple possible):"])
			for k, v in pairs(values) do
	...
	elseif tab.type=="color" then
		...
		if tab.hasAlpha then
			if string.len(str) == 8 and string.find(str,"^%x*$")  then
				...
				r,g,b,a = string.match(str,"^([%d%.]+) ([%d%.]+) ([%d%.]+) ([%d%.]+)$")
