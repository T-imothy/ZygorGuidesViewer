...
local MINOR_VERSION = tonumber(string.match("$Revision: 1 $", "%d+"))

local addon = "LibTaxi-1.0"
local _L = ZGV_LibTaxiLocale
...
do
	...
	if Lib then
		...
		local function addTaxi(name)
			...
			-- Vanilla Lua 5.0 does not provide the later string method syntax
			-- (`name:match`).  Read captures directly from string.find instead.
			local matchStart,matchEnd,nm,zone = string.find(name,"^(.+),%s*(.-)$")
			if nm then name=nm end
			name=Lib.TaxiNames_English[name] or name
			--print(i..": "..TaxiNodeName(i).." = "..name)
			if not name then
				print("|cffff8888LibTaxi Error: Untranslated taxi node|r")
				return
			end
		...
		Lib.frame:SetScript("OnEvent", function() onEvent(this,event,arg1) end)
