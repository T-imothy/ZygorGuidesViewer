...
local WotLK = false -- WoW 1.12.1 Classic backport
...
--	if not ZygorGuidesViewerMiniFrame then error("Zygor Guide Viewer step frame not loaded.") end
	...
	-- TEST313: the Vanilla backport replaces the legacy XML interface, so its
	-- maintenance frame may intentionally not exist.  Some dead-login clients
	-- also report Shift during early initialization.  Enter maintenance mode
	-- only when the complete legacy frame is actually available; otherwise use
	-- the normal guide startup path.
	local maintenanceRequested = IsShiftKeyDown and IsShiftKeyDown()
	if maintenanceRequested and ZygorGuidesViewerMaintenanceFrame then
	...
	self.db.char.completedQuests = self.db.char.completedQuests or {}
	self.completedQuests = self.db.char.completedQuests
...
function me:AddEvent(event,func)
	...
	if not self.EventFrame then
		self.EventFrame = CreateFrame("Frame", "ZygorClassicEventFrame", UIParent)
		self.EventFrame.owner = self
		self.EventFrame:SetScript("OnEvent", function()
			if this.owner then
				this.owner:EventHandler(event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9)
			end
		end)
	end

	if ZGV_len(self.Events[event])==1 then
...
function me:SetGuide(name,step,temp)
	...
	if guide and guide.steps then
		...
		self:Print(string.format(L["message_loadedguide"],name))
		...
		if ZGV_len(self.CurrentGuide.steps)<step then
			step = 1
		end
		...
		self:Print(string.format(L["message_missingguide"],name))
...
function me:FocusStep(num,quiet)
	...
	if num>ZGV_len(self.CurrentGuide.steps) then return end
	...
	if not found then
		...
		if ZGV_len(history)>self.db.profile.guidesinhistory then tremove(history,1) end
	end
...
function me:UpdateLocking()
	-- Classic 1.12 native viewer: titlebar/resizer XML controls do not exist.
	if self.Frame then
		self.Frame:EnableMouse(not self.db.profile["windowlocked"])
	end
end
...
function me:UpdateFrame(full,onupdate)
	...
	if self.loading then
		...
		ZygorGuidesViewerFrame_MissingText:SetText(string.format(L['miniframe_loading'],(self.loadprogress or 0)*100))
	...
	elseif self.db.profile.displaymode=="guide" then
		...
		if self.CurrentGuide and self.CurrentGuide.steps then
			...
			if full then
				ZygorGuidesViewerFrameScrollScrollBar:SetMinMaxValues(1,ZGV_len(self.CurrentGuide.steps)>0 and ZGV_len(self.CurrentGuide.steps) or 1)
			...
			local laststep = self.db.profile.showallsteps and ZGV_len(self.CurrentGuide.steps) or self.CurrentStepNum+self.db.profile.showcountsteps-1
			...
			for stepbuttonnum = 1,self.StepLimit do repeat
				...
				if stepnum>=firststep and stepnum<=laststep and stepnum<=ZGV_len(self.CurrentGuide.steps) then
					...
					if stepdata.requirement or self.db.profile.stepnumbers then
						local numbertext = self.db.profile.stepnumbers and string.format(L['step_num'],stepnum)
						local reqtext = stepdata.requirement and ((stepdata:AreRequirementsMet() and "|cff44aa44" or "|cffbb0000") .. "(" .. (table.concat(stepdata.requirement,L["stepreqor"])):gsub("!([a-zA-Z ]+)",string.format(L["req_not"],"%1")) .. ")")
						local leveltext = (stepdata.level and stepdata.level>0 and self.db.profile.stepnumbers) and string.format(L['step_level'],stepdata.level or "?")
					...
					if stepdata:AreRequirementsMet() or self.db.profile.showwrongsteps then
						...
						for i,goal in ipairs(stepdata.goals) do
							...
							if goal:GetStatus()~="hidden" then
								...
								local indent = string.rep("  ",goal.indent or 0)
								...
								-- not anymore
								--[[
								if goal.orlogic and i<ZGV_len(stepdata.goals) and stepdata.goals[i+1].orlogic then
									frame.lines[line].label:SetFont(FONT,math.round(self.db.profile.fontsecsize))
	...
	elseif self.db.profile.displaymode=="gold" then
		...
		if ZGV_len(spots)>0 then
			...
			if ZGV_len(self.registeredmapspotsets)>0 then
		...
		for spotbuttonnum = 1,self.StepLimit do repeat
			...
			-- show this button at all?
			if spotnum>=firstspot and spotnum<=lastspot and spotnum<=ZGV_len(spots) then
				...
				frame.lines[line].label:SetText(string.format("|cffffbb00%s|r",spotdata.title))
				line=line+1

				frame.lines[line].label:SetFont(FONT,math.round(self.db.profile.fontsize))
				frame.lines[line].label:SetText(string.format("|cffffff00%s %s,%s|r",spotdata.map,spotdata.x,spotdata.y))
				line=line+1

				if (spotdata.desc) then
					frame.lines[line].label:SetFont(FONT,math.round(self.db.profile.fontsize))
					frame.lines[line].label:SetText(string.format("%s",spotdata.desc))
					line=line+1
				end
				...
				if spotdata.objects then
					...
					for i,obj in ipairs(spotdata.objects) do
						if not obj.hidden then
							if ZGV_len(str)>0 then str=str..", " end
							if obj.toohard then
								str = str .. string.format("|cffff0000%s|r",obj.string)
							else
								str = str .. string.format("|cff00ff00%s|r",obj.string)
							end
						end
					end
					if ZGV_len(str)>0 then
						frame.lines[line].label:SetFont(FONT,math.round(self.db.profile.fontsecsize))
				...
				--self:Print(string.format("spot %d, height %s",spotbuttonnum,height))
	...
	if self.CurrentStep then	-- hey, it may be missing, if the whole guide is for another class
		...
		for i=1,20,1 do  -- update all lines
			...
			if line.goal then
				...
				if not InCombatLockdown() then -- and not stepframe.slideup:IsPlaying()
				...
				elseif status=="incomplete" then
					...
					icon:SetIcon(actionicon[goal.action])
					icon:SetDesaturated(false)
					--if anim_w2r:IsDone() then -- or not anim_w2r:IsPlaying()
				...
				elseif status=="complete" then
					...
					icon:SetIcon(3)
					icon:SetDesaturated(false)
					--if anim_w2g:IsDone() then -- or not anim_w2g:IsPlaying() 
...
function me:ReanchorFrame()
	...
	if upsideup then
		...
		if frame.SetClampRectInsets then frame:SetClampRectInsets(0,0,-25,0) end
	else
		...
		if frame.SetClampRectInsets then frame:SetClampRectInsets(0,0,0,25) end
	end
end
...
function me:AlignFrame()
	...
	self:Debug(string.format("%.2f scale: left %.2f, top %.2f, bottom %.2f, right %.2f",scale,left,top,bottom,right))

	-- regain 100% scale
	left=left*scale  right=right*scale  bottom=bottom*scale  top=top*scale  width=width*scale

	self:Debug(string.format("Scaled: left %.2f, top %.2f, bottom %.2f, right %.2f",left,top,bottom,right))
	...
	--self:Debug(string.format("Now %.2f scale: left %.2f, top %.2f, bottom %.2f, right %.2f",scale,left,top,bottom,right))
...
function me:UpdateSkin()
	-- Classic 1.12 native viewer uses the built-in tooltip backdrop.
	return
end
...
function me:ResizeFrame()
	...
	--self:Debug(string.format("%d %d",left,bottom))
...
function me:IsVisible()
	return self.Frame and self.Frame:IsVisible() or false
end
...
function me:ToggleFrame()
	self.Frame = self.Frame or ZygorGuidesViewerFrame
	if not self.Frame then
		if DEFAULT_CHAT_FRAME then DEFAULT_CHAT_FRAME:AddMessage("Zygor: viewer frame is not loaded.") end
		return
	end
...
function me:RaceClassMatch(fit,dkfix)
	...
	fit=string.upper(fit)
	local neg=false
	if string.sub(fit,1,1)=="!" then
		neg=true
		fit=string.sub(fit,2)
	end
...
function me:SkipStep(delta,fast)
	...
	repeat
		...
		if i<1 then
			...
			--]]
			i=ZGV_len(self.CurrentGuide["steps"])
		end
		if i>ZGV_len(self.CurrentGuide)["steps"] or (delta>0 and self.CurrentStep.finish) then
...
function me:AnimateGears()
	if ZygorGuidesViewerFrame_Border:IsVisible() and false then
...
local function dumpquest(quest)
	local s = string.format("%d. \"%s\" ##%d (lv=%d%s):\n",quest.index,quest.title,quest.id,quest.level,quest.complete and ", complete" or "")
	for i,goal in ipairs(quest.goals) do
		s = s .. string.format("... %d. \"%s\" (%s, %s/%s%s)\n",i,goal.leaderboard,goal.type,goal.num,goal.needed,goal.complete and ", complete" or "")
	end
	return s
end
...
function me:Frame_OnHide()
	...
	and not string.find(debugstack(),"TOGGLEWORLDMAP") -- UGLY hack
	then
...
function me:GoalOnClick(goalframe,button)
	...
	if button=="LeftButton" then
		...
		elseif goal.questid then
			...
			if ZGV_len(mentioned)>1 then
				local s=""
				for i=2,ZGV_len(mentioned) do
					if ZGV_len(s)>0 then s=s.."\n" end
...
function me:GoalOnEnter(goalframe)
	...
	if goal.x and goal.y and goal.map then
		...
		if self.db.profile.windowlocked or goal.force_noway then
			wayline = string.format(L['tooltip_waypoint_coords'],goal.map.." "..goal.x..";"..goal.y)
		else
			wayline = string.format(L['tooltip_waypoint'],goal.map.." "..goal.x..";"..goal.y)
		end
	end
...
local function insert_guides(arr,guides)
	...
	for i,guide in ipairs(guides) do
		...
		local item = {
			text = guide.step and string.format(L['menu_last_entry'],guide.short or "?",guide.step) or (guide.short or "?"),
...
function me:OpenGuideMenu()
	...
	-- display!
	UIDropDownMenu_SetAnchor(-50, 15, ZGVFMenu, "TOPRIGHT", ZygorGuidesViewerFrame_Border_TitleBar, "BOTTOMRIGHT")
	--local backdrop = DropDownList1:GetBackdrop()
	--backdrop.edgeSize=16
	--DropDownList1:SetBackdrop(backdrop)
	EasyMenu(menu,ZGVFMenu,nil,30,10,"MENU",3)
	UIDropDownMenu_SetWidth(300, ZGVFMenu)
end
...
local function split(str,sep)
	local fields = {}
	str = str..sep
	string.gsub(str,"(.-)"..sep, function(c) tinsert(fields, c) end)
	return fields
end
...
local function FindGroup(self,title)
	...
	for i=1,ZGV_len(path) do
		...
		if not found then
			...
			group=group.groups[ZGV_len(group.groups)]
		end
	end
	return group
end
...
function me:RegisterGuide(title,data,extra)
	local group,tit = string.match(title,"^(.*)\\+(.-)$")
	if group then
	...
	tinsert(group.guides,{full=title,short=tit or title,num=ZGV_len(self.registeredguides)+1})
...
function me:RegisterMapSpots(title,data)
	...
	tinsert(group.guides,{full=title,short=tit or title,num=ZGV_len(self.registeredmapspotsets)+1})
...
function me:OnGuidesLoaded()
	...
	self:Print(string.format(L['welcome_guides'],ZGV_len(self.registeredguides)))
...
function me:ParseGuides()
	...
	if ZGV_len(self.registeredguides)>0 then
		for i,guide in ipairs(self.registeredguides) do
			if guide.rawdata then
				...
				if status and parsed then
					...
					guide.rawdata=nil
					self.loadprogress = i/ZGV_len(self.registeredguides)
				else
					if not status then err=parsed end
					if err then
						self:Print(string.format(L["message_errorloading_full"],guide.title,line or 0,linedata or "???",err))
					else
						self:Print(string.format(L["message_errorloading_brief"],guide.title))
					end
					guide.rawdata=nil
				end
				self:UpdateFrame(true)
	...
	if ZGV_len(self.registeredmapspotsets)>0 then
		for i,guide in ipairs(self.registeredmapspotsets) do
			if guide.rawdata then
				...
				if status then
					self.loadprogress = i/ZGV_len(self.registeredmapspotsets)
					guide:Show()
				else
					if not status then err=parsedset line=0 linedata="" end
					if err then
						self:Print(string.format(L["message_errorloading_full"],guide.title,line,linedata,err))
					else
						self:Print(string.format(L["message_errorloading_brief"],guide.title))
					end
					guide.rawdata=nil
...
local function superconcat(table,glue)
	local s=""
	for i=1,ZGV_len(table) do
		if ZGV_len(s)>0 then s=s..glue end
...
function me:BugReport(maint)
	...
	if ZygorGuidesViewerMaintenanceFrame then
		HideUIPanel(ZygorGuidesViewerMaintenanceFrame)
	end

	local s = ""
	s = string.format("Zygor Guides Viewer v%s\n",self.version)
	s = s .. "\n"
	s = s .. string.format("Guide: %s\nStep: %d\n",tostr(self.CurrentGuideName),tostr(self.CurrentStepNum))
	...
	if step then
		...
		s = s .. string.format("  (completed: %s, auxiliary: %s, obsolete: %s)\n",step:IsComplete() and "YES" or "no", step:IsAuxiliary() and "YES" or "no", step:IsObsolete() and "YES" or "no")

		s = s .. "Goals: \n"

		for i,goal in ipairs(step.goals) do
			s = s .. string.format("%d. %s %s\n",i,(". "):rep(goal.indent),goal.text and "\""..goal.text.."\"" or "<"..goal:GetText()..">")
			...
			if goal.x or goal.y or goal.action=="goto" then
				s = s .. string.format("    map: %s %s,%s",goal.map or "unknown",goal.x or "nil",goal.y or "nil")
				if goal.dist then s = s .. string.format("  +/- %s",goal.dist) end
				s = s .. "\n"
			end
			if goal.useitemid or goal.useitem then
				s = s .. string.format("   useitem: \"%s\"  ##%s",tostring(goal.useitem),tostring(goal.useitemid))
				if goal.useitemid then
					local a={GetItemInfo(goal.useitemid)}
					s = s .. string.format("  GetItemInfo(%d) == %s\n",goal.useitemid,superconcat(a,","))
				elseif goal.useitem then
					local a={GetItemInfo(goal.useitem)}
					s = s .. string.format("  GetItemInfo(\"%s\") == %s\n",goal.useitem,superconcat(a,","))
				end
			end
			if goal.castspellid or goal.castspell then
				s = s .. string.format("   castspell: \"%s\"  ##%s",tostring(goal.castspell),tostring(goal.castspellid))
				if goal.castspellid then
					local a={GetSpellInfo(goal.castspellid)}
					s = s .. string.format("  GetSpellInfo(%d) == %s\n",goal.castspellid,superconcat(a,","))
				elseif goal.castspell then
					local a={GetSpellInfo(goal.castspell)}
					s = s .. string.format("  GetSpellInfo(\"%s\") == %s\n",goal.castspell,superconcat(a,","))
				end
			end
			if goal.quest or goal.questid then
				s = s .. string.format("    quest: \"%s\" ##%d",tostring(goal.quest),tostring(goal.questid))
				if goal.questid then
					...
					if goal.objnum then
						if questdata then
							...
							s = s .. string.format(" goal %d: \"%s\"",goal.objnum,goaltext)
						else
							s = s .. string.format(" goal %d",goal.objnum)
						end
					else
						s = s .. (" (no goal)")
					end
			...
			if goal.target then
				s = s .. string.format("    target: \"%s\"",goal.target)
			...
			if goal.questreqs and ZGV_len(goal.questreqs)>0 then
			...
			if goal:IsCompleteable() then
				...
				s = s .. string.format("    (complete: %s, possible: %s, auxiliary: %s, obsolete: %s)\n",comp and "YES" or "no", poss and "YES" or "no", step:IsAuxiliary() and "YES" or "no", step:IsObsolete() and "YES" or "no")
			else
	...
	s = s .. string.format("Race: %s  Class: %s  Level: %d\n",select(2,UnitRace("player")),select(2,UnitClass("player")),UnitLevel("player"))
	local x,y = GetPlayerMapPosition("player")
	s = s .. string.format("Position: realzone:'%s' x:%g,y:%g (zone:'%s' subzone:'%s' minimapzone:'%s')\n",GetRealZoneText(),x*100,y*100,GetZoneText(),GetSubZoneText(),GetMinimapZoneText())
	if GetLocale()~="enUS" then
		s = s .. string.format("    enUS: realzone:'%s' zone:'%s' subzone:'%s' minimapzone:'%s')\n",BZR[GetRealZoneText()],BZR[GetZoneText()],BZR[GetSubZoneText()] or "("..GetSubZoneText()..")",BZR[GetMinimapZoneText()] or "("..GetMinimapZoneText()..")")
		s = s .. string.format("Locale: %s\n",GetLocale())
	end
	s = s .. "\n"
	...
	for bag=-2,4 do
		for slot=1,GetContainerNumSlots(bag) do
			...
			if item then
				...
				tinsert(inventory,string.format("    %s ##%d x%d\n",name,id,count))
			end
		end
	end
	...
	for i=1,30 do
		...
		if name then s=s..string.format("%s (\"%s\")\n",name,tex) end
	end
	for i=1,30 do
	...
	for i=1,12 do
		...
		if name then s=s..string.format("%d. %s (\"%s\")\n",i,name,tex) end
	end
	s = s .. "\n"
	...
	self.dumpFrame.title:SetText(string.format(L["report_title"],title,author))
...
function me:DumpVal(val,lev,maxlev,nofun)
	...
	elseif type(val)=="number" then
		s = string.format("%s",tostring(val))
	elseif type(val)=="function" then
		s = ("")
	elseif type(val)=="table" then
		s = "\n"
		for k,v in pairs(val) do
			if type(k)~="string" or not string.find(k,"^parent")
			then
				if type(v)~="function" then
					s = s .. ("   "):rep(lev) .. string.format("%s=%s",k,self:DumpVal(v,lev+1,maxlev,nofun))
				elseif not nofun then
					s = s .. ("   "):rep(lev) .. string.format("%s(function)\n",k)
				end
			end
		end
	end
...
function me:GetQuestData(qid)
	...
	for i = 1,n do
		...
		if i == 1 then
			...
			if line then
				local o, n = string.match(line,"^(.-) x.?.?(%d+)$")
...
function me:GetItemData(itemid,n)
	...
	line = string.gsub(line,"|c%x%x%x%x%x%x%x%x", ""):gsub("|r", ""):gsub("[\n\t]", " ")
...
function me:GetTranslatedNPC(num)
	...
	local name,desc = string.match(s,".|(.-)|(.*)")
...
function me:PruneNPCs()
	...
	for i,d in pairs(ZygorGuidesNPCs) do
		if string.sub(d,1,1)==badf then ZygorGuidesNPCs[i]=nil end
	end
end
