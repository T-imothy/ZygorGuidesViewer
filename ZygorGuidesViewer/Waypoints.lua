...
local addonnum = {}
for i=1,ZGV_len(addonnames) do addonnum[addonnames[i]]=i end
...
function me:SetWaypointAddon(info,addon)
	...
	addon=string.gsub(addon,"[0-9]-_","")

	self:Debug("Setting waypoint addon: "..addon)
	if addon~="none" and not self:IsWaypointAddonReady(addon) then
		self:Print(string.format(L["waypointaddon_fail"],L["opt_group_addons_"..addon]))
		return
	end
	...
	self:Print(string.format(L["waypointaddon_set"],L["opt_group_addons_"..addon]))
...
me.WaypointFunctions['internal'] = {
	...
	setwaypoint = function (self,goalnumORx,y,title)
		...
		if not y then
			...
			if goalnumORx then goals={self.CurrentStep.goals[goalnumORx]} else for i=1,ZGV_len(self.CurrentStep.goals) do if self.CurrentStep.goals[i].x then tinsert(goals,self.CurrentStep.goals[i]) end end end
			for k,goal in ipairs(goals) do
				if not goal.force_noway then
					local way = self.Pointer:SetWaypoint (nil,goal.map,goal.x,goal.y,{title=title or self.CurrentStep:GetTitle() or (goal.map and goal.x and ("%s %d,%d"):format(goal.map,goal.x,goal.y)) or string.format(L['waypoint_step'],self.CurrentStepNum),onminimap="always",overworld=true})
					if way then
...
function me:SetWaypoint(...)
	...
	if unpack(arg)~=false and self.db.profile.hidearrowwithguide and not ZGV.Frame:IsShown() then return end
	if not self:IsWaypointAddonReady() then
		self:Print("Waypoint addon '"..self.db.profile.waypointaddon.."' failed.")
		return
	end
	self.ConnectedWaypointer.setwaypoint(self,unpack(arg))
end
...
function me:UnsetWaypointAddon()
	...
	self:Print(string.format(L["waypointaddon_disconnected"],L["opt_group_addons_"..addon]))
end
...
function me:qRegisterNotes()
	...
	if self.iconregistryretries==3 then
		self:Print(string.format(L["waypointaddon_fail"],L["opt_group_addons_"..self.db.profile.waypointaddon]))
	...
	--self:Print(string.format(L["waypointaddon_connecting"],self.optionsmap.args.waypoints.values[self.db.profile.waypointaddon]))
					...
					if (TomTom:GetMapFile(contid,zoneid)) then
						for note,mapnote in pairs(self.MapNotes[zone]) do
							...
							if x and y then
								...
								self.TomTomWaypoints[ZGV_len(self.TomTomWaypoints)+1] = TomTom:AddZWaypoint(
	...
	elseif addon=="cart2" then
		--[[
		self:Debug("registering database "..ZGV_len(self.MapNotes))
	...
	self:Print(string.format(L["waypointaddon_connected"],L["opt_group_addons_"..addon]))
...
function me:CreateCartographerWaypoints(goalnum)
	...
	for i=ZGV_len(self.CurrentStep.goals),1,-1 do
		...
		if g.x and not g.force_noway then
			...
			if zone then
				...
				db[zone][note]={icon="Circle",title=g.title or self.CurrentStep.title or g.autotitle or self.CurrentStep:GetTitle() or string.format(L['waypoint_step'],self.CurrentStepNum)}
...
function me:GetMapZoneFile(zonename)
	Astrolabe = DongleStub("Astrolabe-0.4-Zygor")
	for cont in pairs{GetMapContinents()} do
		for zone,name in pairs{GetMapZones(cont)} do
			if name==zonename then
