...
local _learnRecipeMsg = ERR_LEARN_RECIPE_S or "You have learned how to create a new item: %s."
local ERR_LEARN_RECIPE_S_fmt = string.gsub(string.gsub(_learnRecipeMsg,".","%."),"%%s","(.+)")
--local TRADESKILL_LOG_FIRSTPERSON_fmt = TRADESKILL_LOG_FIRSTPERSON:gsub("%%s","(.-)")

function me:Profession_CHAT_MSG_SYSTEM(event,text)
	local _,_,item = string.find(text,ERR_LEARN_RECIPE_S_fmt)
	if item then
