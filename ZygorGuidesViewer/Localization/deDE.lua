...
ZygorGuidesViewer_L("Specials", "deDE", function() return {
	...
	['contract_mobs_old'] = function(mobs)
			...
			if not mobs[1].name and type(mobs)=="table" then
				local l=mobs
				mobs={}
				for i=1,ZGV_len(l) do mobs[i]={name=l[i]} end
			end
			local common,all
			for i=1,50 do
				common = mobs[1].name:sub(1,i)
				all=true
				for m=2,ZGV_len(mobs) do
			...
			if ZGV_len(start)>ZGV_len(ending) and ZGV_len(start)>3 then
				return ZygorGuidesViewer_L("Specials")['contract_mobs_start'](start)
			elseif ZGV_len(ending)>ZGV_len(start) and ZGV_len(ending)>3 then
