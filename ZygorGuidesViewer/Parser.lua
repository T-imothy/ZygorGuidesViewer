...
--	if dist then dist=dist/100 or 0.2 end
	...
	if map and ZGV_len(map)<5 then map=nil end
...
function me:ParseHeader(text)
	...
	while (index<ZGV_len(text)) do
		...
		local cmd,params = string.match(line,"([^%s]*)%s?(.*)")
...
local function MakeCondition(cond,forcebool)
	local s
	if forcebool then s = string.format("_Update()  return not not (%s)",cond)
		     else s = string.format("_Update()  return %s",cond)
		     end
...
function me:ParseEntry(text)
	...
	--local debug
	--if string.find(text,"goto The Exodar,44.9,24.2") then debug=true end
	...
	while (index<ZGV_len(text)) do
		...
		--line = string.gsub(line,"^[%s\t]+","")
		--line = string.gsub(line,"[%s\t]+$","") --done in the find

		--st,en = strfind(line,"//",1,true)
		--if st then line=string.sub(line,1,st-1) end
		-- not really faster
		line = string.gsub(line,"//.*$","")

		local indent
		indent,line = string.match(line,"^(%.*)(.*)")
		...
		for chunk in string.gmatch(line,"%s*(.-)%s*|+") do
			...
			--chunk = string.gsub(chunk,"^turn in ","turnin ")
			chunk = string.gsub(chunk,"^@(%S)","@ %1")
			--chunk = string.gsub(chunk,"^%s+","")
			--chunk = string.gsub(chunk,"[%s	]+$","")
			...
			elseif cmd=="step" then
				step = { goals = {}, map = prevmap, level = prevlevel, num = ZGV_len(guide.steps)+1, parentGuide=guide }
				guide.steps[ZGV_len(guide.steps)+1] = step
	...
	--[[
			elseif cmd=="@" then
				local map,x,y
				map,x,y = string.match(params,"(.+),([0-9.]+),([0-9.]+)")
				if not map then
					x,y = string.match(params,"([0-9.]+),([0-9.]+)")
				end
				if not x then
					map = params
				end
	...
	--]]
			...
			elseif cmd=="accept" or cmd=="turnin" then
				...
				local q,qp = string.match(goal.quest,"^(.-)%s-%((%d+)%)$")
			...
			elseif cmd=="from" then
				...
				for i,mob in ipairs(mobs) do
					local name,plural = string.match(mob,"^(.+)(%+)$")
			...
			elseif cmd=="equipped" then
				goal.action = goal.action or cmd
				local slot,item = string.match(params,"^([a-zA-Z]+) (.*)")
			...
			elseif cmd=="rep" then
				goal.action = goal.action or cmd
				goal.faction,goal.rep = string.match(params,"(.*),(.*)")
			...
			elseif cmd=="skill" or cmd=="skillmax" then
				goal.action = goal.action or cmd
				goal.skill,goal.skilllevel = string.match(params,"^(.+),([0-9]+)$")
			...
			elseif cmd=="only" then
				local cond = string.match(params,"^if%s+(.*)$")
				if cond then
					-- condition match
			...
			elseif cmd=="daily" then
				...
				if ZGV_len(guide.steps)==0 then guide.daily=true end
			...
			elseif cmd=="quest" or cmd=="q" then
				local first=string.match(params,"^(.-),")
			...
			elseif ZGV_len(chunk)>1 then -- text
				-- snag coordinates for waypointing, with distance
				local st,en,x,y,d
				st,en = 1,1

				st,en,x,y,d = string.find(params,"([0-9%.]+),([0-9%.]+)(,([0-9%.]+))?",en)
				if not x then
					-- without distance, perhaps?
					d=0.2
					st,en,x,y = string.find(params,"([0-9%.]+),([0-9%.]+)",en)
				end
				...
				if x and y then
					...
					params = string.sub(params,1,st-1) .. COLOR_LOC(string.format(L['coords'],x,y)) .. string.sub(params,en+1)
				end
		...
		if ZGV_len(TableKeys(goal))>0 then
			...
			goal.num = ZGV_len(step.goals)+1

			step.goals[ZGV_len(step.goals)+1] = goal
		...
		if goal then goal.indent = ZGV_len(indent) end
