...
function Goal:GetStatus()
	...
	if ZGV.db.profile.showobsolete and not self.parentStep.parentGuide.noobsolete and self:IsObsolete() then return "obsolete" end
...
local goalstring_slain=string.gsub(QUEST_MONSTERS_KILLED or "%s slain: %d/%d",": .*","")
...
function Goal:IsComplete()
	...
	elseif self.action=="turnin" then
		local inlog = ZGV.questsbyid[self.questid]
			--[[ Completion sequence:
			 ...
			 1. CHAT_MSG_SYSTEM "<quest> completed."	nil				{unpack(arg)}
			     - ZGV.completedQuests[id] = true		true				{unpack(arg)}
		...
		return turned, turned or (inlog and (inlog.complete or ZGV_len(inlog.goals)==0))
	end
	...
	if self.questid then
		...
		if questInLog then
			...
			if self.objnum then
				...
				--[[
				if questInLog.complete or ZGV_len(questInLog.goals)==0 then
					return true,true
				else
	...
	elseif self.action=="havebuff" then
		for i=1,30 do
			...
			if name and (string.find(tex,self.buff) or string.find(name,self.buff)) then return true,true end
	...
	elseif self.action=="nobuff" then
		for i=1,30 do
			local name,_,tex = UnitBuff("player",i)
			if name and (string.find(tex,self.buff) or string.find(name,self.buff)) then return false,true end
			local name,_,tex = UnitDebuff("player",i)
			if name and (string.find(tex,self.buff) or string.find(name,self.buff)) then return false,true end
		end
		return true,true
	...
	elseif self.action=="equipped" then
		...
		local name
		if link then name = string.match(link,"|Hitem:.-%[(.-)%]") end
...
function FindPetActionInfo(action)
	...
	for i=1,12 do
		...
		if name and (string.find(name,action) or string.find(tex,action)) then return i,name,x,tex end
	end
end
...
function Goal:GetText(showcompleteness)
	...
	elseif self.action=='from' then
		...
		if ZGV_len(self.mobs)>1 then
			...
			if not text then
				...
				text = ""
				for i,mob in ipairs(self.mobs) do
					if ZGV_len(text)>0 then text = text .. ", " end
	...
	and not (self.text and self.string.find(text,"[0-9%.]-,[0-9%.]-")) -- and it's not a coord-in-text
...
function Goal:Prepare()
	if self.castspell or self.castspellid and (not self.castspell or not self.castspellid) then
		...
		if link then
			self.castspellid,self.castspell = string.match(link,"spell:([0-9]+).-%[(.-)%]")
