...
local rawmessages={FACTION_STANDING_DECREASED,FACTION_STANDING_DECREASED_GENERIC,FACTION_STANDING_INCREASED,FACTION_STANDING_INCREASED_BONUS,FACTION_STANDING_INCREASED_GENERIC}
local messages={}
for _,msg in ipairs(rawmessages) do
	if type(msg)=="string" then
		local pattern = string.gsub(msg,"%%[.%d]?[sdf]","(.*)")
		tinsert(messages,pattern)
	end
end
...
function me:CHAT_MSG_COMBAT_FACTION_CHANGE_Faction(event,text)
	...
	for i,msg in ipairs(messages) do
		local faction = string.match(text,msg)
		if faction then
			local rep = self:GetReputation(faction)
			print(string.format("%s: %s (%s)",faction,rep:Current(),rep:Going()))
			return
		end
	end
end
