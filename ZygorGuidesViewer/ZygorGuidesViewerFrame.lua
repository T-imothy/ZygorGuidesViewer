...
-- Vanilla 1.12 can finish addon/library initialization later than newer clients.
-- Do not abort loading this file just because the addon object is not ready yet.
local function ZGV_Get()
	if not ZGV then ZGV = ZygorGuidesViewer end
	return ZGV
end
...
function ZygorGuidesViewerFrame_Step_Setup(num)
	...
	for i=1,20,1 do
		...
		back:ClearAllPoints()
		back:SetPoint("TOPLEFT",line,"TOPLEFT",0,0)
		back:SetPoint("BOTTOMRIGHT",line,"BOTTOMRIGHT",0,0)

		clicker:ClearAllPoints()
		clicker:SetPoint("TOPLEFT",line,"TOPLEFT",0,0)
		clicker:SetPoint("BOTTOMRIGHT",line,"BOTTOMRIGHT",0,0)
		--clicker.num = i
...
--	this:RegisterEvent("PARTY_MEMBER_DISABLE");
	...
	if self.SetClampRectInsets then self:SetClampRectInsets(0,0,10,0) end
	...
	for i=1,20 do
		...
		local action = CreateFrame("CheckButton", name.."Action", UIParent)
		local petaction = CreateFrame("CheckButton", name.."PetAction", UIParent, "PetActionButtonTemplate")

		action._zgvattrs = {}
		function action:SetAttribute(k,v) self._zgvattrs[k]=v end
		function action:GetAttribute(k) return self._zgvattrs[k] end
		action:SetScript("OnClick", function()
			local self=this
			local typ=self:GetAttribute("type1")
			if typ=="spell" then CastSpellByName(self:GetAttribute("spell1"))
			elseif typ=="item" then UseItemByName(self:GetAttribute("item1")) end
		end)
		action:SetFrameStrata("MEDIUM")
		if action.SetToplevel then action:SetToplevel(true) end
		action:SetWidth(15)
		action:SetHeight(15)
		action:SetScript("OnEnter", function(self)
			...
			if self:GetAttribute("type1")=="spell" then
				...
				GameTooltip:ClearAllPoints()
				GameTooltip:SetPoint("BOTTOMRIGHT",self,"TOPLEFT")
				GameTooltip:SetText(self:GetAttribute("spell1") or "Spell")
				GameTooltip:Show()
			end
		end)
...
function ZygorGuidesViewerFrame_OnUpdate(self,elapsed)
	...
	local but = ZygorGuidesViewerFrame_Border_Gears_GuideButton
