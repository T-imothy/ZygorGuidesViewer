...
function me:Options_DefineOptions()
	local Getter_Simple = function(info)
		return self.db.profile[info[ZGV_len(info)]]
	end
	local Setter_Simple = function(info,value)
		self.db.profile[info[ZGV_len(info)]] = value
	end
	...
	self.options = {
		...
		args = {
			desc = {
				order = 1,
				type = "description",
				name = string.format(L["desc"],self.version),
				order = 1,
			},
			desc = {
				order = 1,
				type = "header",
				name = string.format(L["opt_guide"],self.version),
				order = 1,
			},
			guide = {
				order = 2,
			...
			steps = {
				order=3.1,
				type = "description",
				name = function() if not ZGV.CurrentGuide then return "" end  return string.format(L["opt_guide_steps"],ZGV_len(ZGV.CurrentGuide.steps)) end,
			},
			author = {
				order=3.2,
				type = "description",
				name = function() if not ZGV.CurrentGuide or not ZGV.CurrentGuide.author then return "" end  return string.format(L["opt_guide_author"],ZGV.CurrentGuide.author) end,
			},
			next = {
				order=3.3,
				type = "description",
				name = function() if not ZGV.CurrentGuide or not ZGV.CurrentGuide.next then return "" end  return string.format(L["opt_guide_next"],ZGV.CurrentGuide.next) end,
			},
			show = {
				name = L["opt_visible"],
	...
	self.optionsdisplay = {
		...
		args = {
			...
			guidesinhistory = {
				...
				min = 3,
				max = 15,
				set = function(i,v) Setter_Simple(i,v)  while (ZGV_len(self.db.char.guides_history)>v) do tremove(self.db.char.guides_history) end   end,
				step = 1,
				bigStep = 1,
				order=2.8
			},
	...
	self.optionsprogress = {
		...
		args = {
			...
			showobsolete = {
				...
				type = 'toggle',
				set = function(i,v) Setter_Simple(i,v)  if not v then self.db.profile.skipobsolete=false end  self:UpdateFrame()  end,
				width = "full",
				order = 3.6,
			},
			skipobsolete = {
			...
			_internal = {
				...
				args = {
					...
					mapcoords = {
						...
						set = function(i,v)
							Setter_Simple(i,v)
							-- TEST314: MapCoords is an optional later-client module and is
							-- intentionally absent from the single-addon Vanilla package.
							if self.MapCoords and self.MapCoords.HandleWorldmapCoords then
								self.MapCoords:HandleWorldmapCoords()
							end
						end,
						width = "full",
						order = 10.23,
	...
	self.optionsdebug = {
		...
		args = {
			...
			fakelevel = {
				...
				step = 1,
				bigStep = 1,
				get = function(i,v) return self.db.char[i[ZGV_len(i)]] end,
				set = function(i,v) self.db.char[i[ZGV_len(i)]]=v end,
				width="double",
				order = 3.9
			}
		},
...
function me:Options_SetupBlizConfig()
	-- WoW 1.12 has no modern InterfaceOptionsFrame/AceConfig Blizzard panel.
	return
end
...
function me:OpenOptions()
	--self:OpenConfigMenu()
	InterfaceOptionsFrame_OpenToFrame(L['name'])
end
